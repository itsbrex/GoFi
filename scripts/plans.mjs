...
//   bun run plans app <slug> [--title "T"] [--badge "TAG"] [--dest dir] [--template name]
//                                 # stamp a single-file skeleton APP (default template:
//                                 # the extracted SF-Ownership-Desk design system — Geist
//                                 # mono, ⌘K command bar, 10 themes, drawer/modal/toasts).
//                                 # Writes <dest>/<slug>.html (default apps/). Named
//                                 # templates (.app-template-<name>.html) are optional extras.
...
/* ---------------- app skeleton ---------------- */
// Stamp a self-contained skeleton app from the DEFAULT app template
// (docs/plans/.app-template.html, hook-synced from ~/.claude/templates/plans/).
// `--template <name>` selects an optional alternative (.app-template-<name>.html)
// when one exists; the unnamed skeleton stays the default.
async function cmdApp(argv) {
  const { flags, rest } = parseFlags(argv);
  const slug = (rest[0] || '').toLowerCase().replace(/[^a-z0-9-]+/g, '-').replace(/^-+|-+$/g, '');
  if (!slug) {
    console.error('Usage: plans app <slug> [--title "Title"] [--badge "TAG"] [--dest dir] [--template name]');
    process.exit(1);
  }
