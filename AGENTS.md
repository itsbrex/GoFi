...
3. **Download Engine**: `download/download.go` handles the actual download of music files
   ...
   - Smart file existence checking to avoid re-downloads:
     - Uses `os.Lstat()` to detect files without following symlinks
     - Automatically skips existing files (regular files, symlinks, or macOS aliases)
     - Checks both custom filenames and default ID-based naming schemes
